...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/testng-results">
        ...
        <xsl:result-document href="{testng:absolutePath('overview.html')}" format="xhtml">
			...
			<xsl:for-each select="$suiteElements">
				<h2>Suite <i><xsl:value-of select="@name" /></i> started at <xsl:value-of select="@started-at" /> and finished at  <xsl:value-of select="@finished-at" />. <br />
			Total duration(in ms) was <xsl:value-of select="testng:formatDuration(./@duration-ms)" /></h2>
			</xsl:for-each>
                    <table width="100%">
                        <tr>
			  ...
			  <td align="center">Reporter Log</td>
			       ...
			       <td align="center">
				<xsl:for-each select="class/test-method/reporter-output/line">
					<div><code>
    ...
    <xsl:template name="suiteTestCasesLinks">
		...
		<xsl:value-of select="class/test-method/reporter-output/line" disable-output-escaping="yes"/>
            </div>
        </xsl:for-each>
    </xsl:template>
